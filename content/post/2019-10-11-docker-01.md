...
2. **Enable Kubernetes** のチェックボックスを選択し, **Apply** を押す.<br>`Docker Desktop`が`Kubernetes`のセットアップを行い, 準備が完了すると **Kubernetes Running** の緑色のランプが光る.  
...
6. `pod`のログを表示し, `ping`のログが表示されることを確認する.  
```bash
# pod(demo)のログを確認する
$ kubectl logs demo
...
2. `8.8.8.8`に`ping`を送る`service`を起動する.  
[追加]`service`についてはPart3,4で説明.  
```bash
# alpine(image)を使用して8.8.8.8にpingを送るservice(demo)を起動する
